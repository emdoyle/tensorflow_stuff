...
num_targets = len(constants.TARGET)
...
correct_by_attr = {k: 0 for k in constants.TARGET}
incorrect_by_attr = {k: 0 for k in constants.TARGET}
...
if len(predictions_lines) == len(truth_lines):
	for x in range(START, len(predictions_lines)):
		all_correct = True
		for y in range(0, len(constants.TARGET)):

			attribute = constants.TARGET[y]
